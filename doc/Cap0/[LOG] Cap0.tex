...
programação funcional, procedural e etc., mas, em princípio, lidaremos com ela como uma linguagem: Proloag (que vem de \foreign{``Programmation Logique''}). Isso nos
...
Como dizia Sussman %TODO citar direito
...
Para nossos propósitos, é uma simplificação razoável dizer que funtores expressam relações. Na linguagem natural, somos restritos a usar os funtores presentes na língua usada (mesmo uma ``licensa poética'' não vai muito longe disso).
...
\noindent diz que ``é verdade que mais(1,2,3)'' ou, mais naturalmente, ``é verdade que existe a relação 'mais' entre 1, 2 e 3, nessa ordem'' (vale notar que a ordem é importante: {\tt mais(1,2,3)} não é o igual a {\tt mais(3,2,1)}, onde por ``igual a'' entende-se ``idêntico a'').
...
como {\tt 1 f 2}. É possível definir funtores dessa forma, mas nos restringiremos ao modo usual, pelo qual a última relação fica
{\tt f(1,2)}.

Eventualmente pode ser que, na ocasição de a relação {\tt p(a,b)} ser verdadeira outras relações também sejam.
...
\lstinputlisting[caption=Pai e filho, style=prosty]{../Exemplos/Cap0/prog4_pai.pl}
\label{lst:pai_filho}
...
Com essa discussão em mente, será ocasionalmente útil ter o \enphasis{significado} de um programa lógico definido de forma algo mais precisa:
  ...
  significado do programa é {\tt pai(d,c)?}.
