...
\end{itemize}
  ...
  associação quando o seu \technical{car} é uma variável.
  ...
  \foreign{place-holder}, e, quando não apresentar valor, podemos
  ...
  \technical{reify-name}\footnote{$string\rightarrow symbol$ é escrito
  ...
  único em cada uso de \technical{reify-name}.

  Para continuar com nosso esquema de reificação, vamos precisar de
  uma versão ligeiramente diferente de \technical{walk}, o qual
  ...
  variável com associação na substituição \enphasis{s}. Além disso,
  note que, se \technical{$walk^*$} produz um valor \enphasis{v} ao
  \enphasis{caminhar} por uma substituição \enphasis{s}, temos
  garantia de que as variáveis em \enphasis{v} são frescas.
  ...
  Para termos essas definições, precisamos de \technical{$take^\infty$},
