...
\foreign{choice points} a mais, o que deve ser evitado (em outras
...
\foreign{choice points}, o que seria vantajoso.
...
Veremos, então, métodos para lidar com essas situações. O mais
...
O símbolo ``\_'' representa uma \definicao{variável anônima}. A
...
cumpre um papel meramente formal, de \foreign{placeholder}). Toda
...
elemento de \var{Xs}\footnote{Estamos assumindo tacitamente, neste
  ...
  baixo, da direita para a esquerda'' (o interpretador ``enxerga''
...
elemento pertence a lista, assim que é obtido sucesso, o processo
para.
...
\codigo{member(c,[a,b,c])?} não estão mais no significado. Perceba que
...
lógicos\footnote{Você pode se perguntar se algum do programa nesse
  ...
  \enphasis{Natural}, do Capítulo 2, %COMMENT: lembrar de colocar
...
respectivamente, o ``se, então, senão'' e o ``Ou''\footnote{Na
  ...
  \technical{backtracking} como o nosso ``ou'' natural, mas às vezes
  pode ser conveniente usar um ``ou'' em uma cláusula, seja por efeito
  de legibilidade ou para evitar o \technical{backtracking}.}, mas
...
\technical{choice points} e \technical{backtrackings}) não é a única
