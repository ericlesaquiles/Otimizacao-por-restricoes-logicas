...
Vale notar também outro detalhe importante naquele algoritmo: ele realiza um procedimento que frequentemente fazemos ao buscar resolver um problema, isto é, transformando uma pergunta, potencialmente complicada e difícil, em uma trivial, para a qual sabe-se a resposta. Essa é uma instância de um processo de \textbf{normalização} \marginpar{\small  \textbf{Normalização}}, isto é, um processo de transformar uma restrição em outra equivalente, mas mais tratável. Processos de normalização frequentemente
...
Esse grafo é uma outra forma de representar a restrição {\tt V=V1, V=V2, V1=I1*R1, V2=I2*R2, I=I1+I2}\footnote{Exemplo adaptado de \cite{marriot}, pág. 36.}. Considere as restrições adicionais de que {\tt V=8, R1=6, R2=44}. Na propagação local, podemos considerar as arestas como veículos das restrições, deixando-as passar de nó em nó de forma apropriada, passo a passo. Por exemplo, dadas as restrições acima, a restrição de {\tt V=8} pode ser propagada para {\tt V1=8}, e teríamos:
...
\begin{center}
  \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=1.5cm, thick,main node/.style={circle,draw,font=\sffamily\bfseries}]
    ...
    \node[main node] (tm) [decision, below of=V1]       {*};
    \node[main node] (tm1)[decision, below of=V2]       {*};
    \node[main node] (R1) [below left of=tm]            {6};
    \node[main node] (R2) [below right of=tm1]          {44};
    \node[main node] (I1) [below of=tm]                 {I1};
    \node[main node] (I2) [below of=tm1]                {I2};
    \node[main node] (pls)[decision, below right of=I1] {+};
    \node[main node] (I)  [below of=pls]                {I};
...
Disso, podemos continuar a propagação, obtendo {\tt I1 = 8/6}. E assim vai.

Propagação local funciona muito bem para resolver alguns problemas, mas simplesmente não consegue resolver outros. Por exemplo, considere a restrição {\tt A=B+2*Z, A=3*K-B, K=5, Z=7}. Propagação local detectaria que {\tt K=5} e {\tt Z=7}, mas não encontraria o valor de A ou B. Isso ocorre porque para tanto seria preciso fazer uso de mais de uma restrição por vez, o que a propagação local não faz.
...
serão explorados. A marcação pode tornar um \textbf{método de busca incompleto}\marginpar{\textbf{Método de busca incompleto/completo}}
...
\begin{definition}
  Uma substituição $\iota$ é dita uma solução parcial\marginpar{\small  \textbf{Solução parcial}} de um CP, que chamaremos C, se existe alguma substituição $\rho$ tal que $\iota \cup \rho$ é uma solução de C.
\end{definition}

\begin{definition}
  Dizemos que uma restrição $R_0$ em função das variáveis $X_0$ a $X_n$ é uma projeção\marginpar{\small  \textbf{Projeção}} da restrição $R_1$, em função das variáveis $X_0$ a $X_m$, com m $>$ n, se toda solução de $R_0$ é uma solução parcial de $R_1$.
\end{definition}
...
\begin{definition}
  Duas restrições\marginpar{\small  \textbf{Restrições equivalentes}} $C_1$ e $C_2$ são equivalentes em relação ao conjunto de variáveis V, o que denotaremos por $C_1 \leftrightarrow C_2$, se toda solução de $C_1$ restrita a V é uma solução parcial de $C_2$ e se toda solução de $C_2$ restrita a V é uma solução parcial de $C_1$.
\end{definition}

\begin{definition}
  Var(X) denota o conjunto de variáveis de um CP X qualquer. Sejam uma restrição composta C e o conjunto de variáveis de interesse inter(C) $\subset$ var(C). Dizemos que C' é uma simplificação\marginpar{\small  \textbf{Simplificação}} de C em inter(C) se inter(C) = var(C) e toda solução de C' é uma solução parcial de C.
\end{definition}
...
\subsection{Equivalência}
  ...
  \begin{definition}
    Seja \textit{simp} uma função que recebe um CP e um conjunto V de variáveis de interesse e retorna um CP. Dizemos que \textit{simp} realiza um processo de \textbf{normalização canônica}\marginpar{\small  \textbf{Normalização canônica}} em um dado domínio se para todo CP C no domínio e todo V $\subset$ var(C), simp(C,V) é uma simplificação de C e se $C_1 \leftrightarrow C_2 \Rightarrow simp(C_1,V) = simp(C_2,V)$.
  \end{definition}
...
%COMMENT: eu colocaria alguma coisa antes dessa pergunta. Tipo "fica a questao:", ou "Surge a pergunta:", ou "Exercicio:". Porque senao parece que vai escrever mais alguma coisa depois dela.
...
para ser tomado como base para programação por restrições. Um primeiro motivo já deve ser claro, nomeadamente que programação lógica é feita com base em um tipo importante de restrição, a em árvores, o que dá à programação lógica e, por extensão, à programação
...
Assim, uma linguagem de programação lógica \textit{vanilla} seria uma $CLP(\acute{A}rvore)$, na qual as únicas restrições primitivas são igualdade e desigualdade (apesar de que desigualdade só está presente de forma implícita e pode ser implementada com base no símbolo de igualdade).
