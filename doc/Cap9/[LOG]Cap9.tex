...
\codigo{consistente\_por\_no/2} recebe uma lista de pares ``Domínio-Restrição''. Se a restrição for unária, ela checa cada valor do domínio.
...
\subsection{Consistência por limites\footnote{Também conhecido como \foreign{bounds consistency}}}
...
\begin{definition}
  ...
  se para cada $x_i$ no domínio de \var{$X_i$}, existem $x_j$ nos domínios de \var{$X_j$} tal que
...
Dizemos que temos um \definicao{CSP é aritmético} se o domínio de cada variável é uma união
...
\begin{definition}
  ...
  se, para cada \var{$X_i$}, existem \enphasisb{valores inteiros} $x_j$ no domínio de \var{$X_j$}, onde $j \neq i$, tais que \{\var{$X_k$} = $x_k$ : $1 \leq k \leq n$ \}
...
\begin{definition}
  ...
  se, para cada \var{$X_i$}, existem \enphasisb{valores inteiros} $x_j$, onde $j \neq i$,
...
\begin{definition}
  ...
  se, para cada \var{$X_i$}, existem \enphasisb{valores reais} $x_j$, onde $j \neq i$,
  satisfazendo $min_D(\var{$X_j$}) \leq x_j \leq max_D(\var{$X_j$})$,
  tais que \{\var{$X_k$} = $x_k$ : $1 \leq k \leq n$ \}
...
valores que essa variável pode receber. Por exemplo, para \codigo{r(X,Y) :- X = Y.}, onde \var{X}
...
essas são muito fáceis de lidar, além de serem equivalentes para \boundd, \boundz{} e \boundr.} e de desigualdades do tipo $x_1 \times x_2 \leq x_3$, onde $min_D(x_i) \geq 0$, são
...
\caption{Consistencia por limites}\label{lst:bounds}
    \end{listing}
