...
restrições\footnote{Por conveniência, usamos $X_i$ no lugar de $X[i]$.} \codigo{$X_i$ - $X_j$ $\setminus=\setminus$ I - J} e \codigo{$X_i$ - $X_j$ %COMMENT: troquei o footnote de lugar porque ele estava parecendo uma potencia
...
mas, adotando um tempo limite de 50 segundos, \var{N} = 30 já está fora de mão. %COMMENT: o que quer dizer com "fora de mao"? Que nao eh mais possivel?
...
essa heurística de \technical{middle\_out}: ela faz com que a busca comece do meio e vá alternando
...
\subsection{Heurísticas de ordenamento de valor} %QUESTION: nao deveria ser "ordenacao"?
...
pelos menores e maiores valores, respectivamente (usando a ordem do domínio, pela qual ``menor'' é o
...
O problema das $n$-rainhas é simétrico, já que se uma solução tem a $n$-ésima rainha no $m$-ésimo quadrado da
coluna $i$, outra solução teria a $n$-ésima rainha no $m$-ésimo quadrado da linha $i$. Perceba que uma solução é
...
Se os valores para uma coluna são excluídos, exceto um, essa variável é instanciada ao valor
