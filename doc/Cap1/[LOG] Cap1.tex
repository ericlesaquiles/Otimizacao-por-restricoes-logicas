...
\begin{itemize}
    ...
    \item[(b)] Se a pilha estiver vazia, devolva sucesso. Se não, faça o \textit{pop} de um elemento (uma equação) $T_1 = T_2$ da pilha. Realize uma das ações a seguir, segundo a equação retirada:
      \begin{enumerate}
        ...
        \item Analogamente se $T_2$ for uma variável e $T_1$ for um termo não contendo $T_2$; %QUESTION: e o que faz neste caso?
...
mas assumiremos que retorna). %COMMENT: esse pedaco de frase sobrou aqui.
...
O item 2 %QUESTION: que item 2? O passo b.2?
