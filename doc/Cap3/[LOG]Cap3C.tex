...
comportamento é diferente do \funtor{=/2}, por exemplo. Assim, o seguinte
...
A diferença entra as listas \var{Ys} e \var{Xs}, com \codigo{Ys = [Zs|Xs]}, é denotada \codigo{Ys$\setminus$Xs}, em que \var{Ys} é dita a \enphasis{cabeça} e \enphasis{Xs} a \enphasis{cauda}. Na prática,
...
%COMMENT: esta bem ruim de acompanhar a que codigo se refere sem ter a legenda...
...
\noindent em que $\bullet$ representa o funtor \funtor{./2} da lista. Na verdade, funtores em geral são árvores, não só os de lista, mas
funtores de lista tem essa ``cara especial''. Uma lista como \codigo{[[a,b],c]} seria como:

%COMMENT: coloquei uma ! antes do h para reforcar que quer que a figura fique "here". Mas veja que pode acontecer da figura mudar de lugar e o paragrafo anterior ficou sem sentido. Tem um "seria como:" e ai ja comeca outro paragrafo. Por isso sempre eh melhor, ao usar figuras (ou tabelas e coisas do tipo), usar coisas como "como apresentado na Figura \ref{xxx}" do que "como na figura a seguir:".
...
O que \funtor{flatten/2} faz é uma transformação em árvores como essa, transformando uma lista aninhada como a da Figura~\ref{fig:nest_list} em uma
...
operação de soma é associativa à esquerda, o que significa que a operação \codigo{1 + 1 + 1 + 1} é tomada como \codigo{(((1 + 1) + 1) + 1)}.
...
\noindent queremos que isso se torne \codigo{ (1 + (2 +  (3 + 4)))}, dado pela árvore da Figura~\ref{fig:sum_norm}.
...
em que \var{S1} e \var{S2} são somas normalizadas incompletas. Nesse contexto, o número 0 faz o papel da lista vazia e
